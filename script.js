...
const exibir = document.querySelector('#meme')
...
function gerarLegenda(){
    const sentences = [
      ...
      return sentences[0][Math.floor(Math.random() * sentences[0].length)] + 
      sentences[1][Math.floor(Math.random() * sentences[1].length)] +
      sentences[2][Math.floor(Math.random() * sentences[2].length)] +
      sentences[3][Math.floor(Math.random() * sentences[3].length)];
}
...
window.addEventListener('load', function(){
    ...
    function mostraDados(dados){
        ...
        var memediv = document.querySelector("#memes")
        ...
        memes.forEach(meme => {
         ...
         var legenda = document.createElement("h5")
         var imgMeme = document.createElement("img");
         imgMeme.src = url;
         imgMeme.width = 200;
         imgMeme.height = 200;
 
         h2.textContent = name;
         legenda.textContent=(gerarLegenda());

        div.appendChild(h2)
        div.appendChild(imgMeme);
